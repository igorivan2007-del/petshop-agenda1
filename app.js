// ===================================================
// SERVIÇOS OFERECIDOS (nome + duração em horas)
// ===================================================
const SERVICOS = {
...
function criarApiSupabase() {
  ...
  const ok = ({ data, error }) => {
    if (error) {
      const e = new Error(
        error.code === "23505" ? "Já existe um cadastro com esse valor único." : error.message
      );
      throw e;
    }
    return data;
  ...
  return {
    listarTutores: async () => ok(await db.from("tutor").select("id_tutor, nome, cpf").order("nome")),
    ...
    listarAgendamentos: async () =>
      ok(
        await db
          .from("agendamento")
          .select("id_agendamento, data_hora, status, servico, pet(nome, especie, raca, tutor(nome))")
          .order("data_hora")
      ),
...
function criarApiLocal() {
  ...
  return {
    ...
    criarAgendamento: async (a) => {
      const d = ler();
      d.agendamento.push({
        id_agendamento: ++d.seq,
        id_pet: Number(a.id_pet),
...
function formatarCpf(cpfBruto) {
  const cpf = (cpfBruto || "").replace(/\D/g, "").slice(0, 11);
...
// Devolve o conjunto de horas ("08:00", "09:00"...) já ocupadas num dia,
// considerando a duração de cada serviço já agendado (banho = 1h, banho e tosa = 2h)
function horariosOcupados(diaStr, ignorarId) {
  const ocupados = new Set();
  agendamentos
    .filter((a) => a.status === "agendado" && a.id_agendamento !== ignorarId)
    .forEach((a) => {
      const d = parseUTC(a.data_hora);
      if (dataParaStr(d) !== diaStr) return;
      const duracao = SERVICOS[a.servico]?.duracao ?? 1;
      const horaInicio = d.getHours();
      for (let k = 0; k < duracao; k++) ocupados.add(`${pad(horaInicio + k)}:00`);
    });
  return ocupados;
}

// Gera as opções de horário disponíveis para um serviço de determinada duração,
// garantindo que ele caiba dentro do horário de funcionamento e não bata em outro horário ocupado
function gerarOpcoesHorario(select, diaStr, servicoKey, ignorarId) {
  const atual = select.value;
  select.innerHTML = '<option value="">Selecione o horário</option>';
  const duracao = SERVICOS[servicoKey]?.duracao ?? 1;
  const ocupados = diaStr ? horariosOcupados(diaStr, ignorarId) : new Set();

  for (let h = HORA_ABERTURA; h <= HORA_FECHAMENTO - duracao; h++) {
    let livre = true;
    for (let k = 0; k < duracao; k++) {
      if (ocupados.has(`${pad(h + k)}:00`)) livre = false;
    }
    if (livre) select.appendChild(new Option(`${pad(h)}:00`, `${pad(h)}:00`));
  }
...
// CADASTRAR TUTOR
// ===================================================
$("tutor-cpf").addEventListener("input", (e) => {
...
formTutor.addEventListener("submit", (e) => {
  ...
  const cpf = $("tutor-cpf").value.trim();

  if (nome.split(/\s+/).filter(Boolean).length < 2) {
    return aviso("Digite o nome completo (nome e sobrenome).", true);
  }
  if (!cpfValido(cpf)) {
    return aviso("CPF inválido. Confira os números digitados.", true);
  }
  const cpfLimpo = cpf.replace(/\D/g, "");
  const nomeDuplicado = tutoresCache.some((t) => t.nome.trim().toLowerCase() === nome.toLowerCase());
  const cpfDuplicado = tutoresCache.some((t) => (t.cpf || "").replace(/\D/g, "") === cpfLimpo);
  if (nomeDuplicado) return aviso("Já existe um tutor cadastrado com esse nome.", true);
  if (cpfDuplicado) return aviso("Já existe um tutor cadastrado com esse CPF.", true);
...
formAgendamento.addEventListener("submit", (e) => {
  ...
  const servico = selectAgendamentoServico.value;
  ...
  comBotao(formAgendamento, async () => {
    await api.criarAgendamento({ id_pet: selectAgendamentoPet.value, data_hora: iso, servico });
...
async function carregarAgendamentos() {
  try {
    ...
    if ($("agendamento-dia").value) {
      gerarOpcoesHorario($("agendamento-hora"), $("agendamento-dia").value, selectAgendamentoServico.value);
    }
  } catch (e) {
...
// ===================================================
// REAGENDAR (mantém o mesmo serviço, só muda data/hora)
// ===================================================
function abrirModal(ag) {
  idReagendando = ag.id_agendamento;
  $("reagendar-info").textContent = `${ag.pet?.nome ?? "Pet"} — ${SERVICOS[ag.servico]?.label ?? "Banho"} — atual: ${parseUTC(ag.data_hora).toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "short" })}`;
  ...
  gerarOpcoesHorario(horaSelect, diaInput.value, ag.servico, ag.id_agendamento);
...
$("agendamento-dia").addEventListener("change", atualizarHorariosAgendamento);
selectAgendamentoServico.addEventListener("change", atualizarHorariosAgendamento);

$("reagendar-dia").addEventListener("change", () => {
  const ag = agendamentos.find((a) => a.id_agendamento === idReagendando);
  gerarOpcoesHorario($("reagendar-hora"), $("reagendar-dia").value, ag?.servico, idReagendando);
});

gerarOpcoesHorario($("agendamento-hora"), null, selectAgendamentoServico.value);
